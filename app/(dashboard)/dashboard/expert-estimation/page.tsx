...
function ExpertEstimatesContent() {
  ...
  // Default project ID (you can make this dynamic based on selected project)
  ...
  const {
    ...
    projectId: urlFilters.filters.project_id || defaultProjectId,
  ...
  const { experts, loading: expertsLoading } = useExperts({ isActive: true });
  const { workItems, loading: workItemsLoading } = useAllWorkItems();
