import { FiCalendar, FiClock, FiUser, FiEye, FiEdit2, FiTrash2, FiLoader } from 'react-icons/fi';
...
export default function TaskCard({ item, onView, onEdit, onDelete, deletingId }: TaskCardProps) {
    ...
    <div className="p-4 sm:p-6 hover:bg-slate-700/30 transition-colors">
      <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-3 sm:gap-4">
        ...
        <div className="flex items-center gap-1.5 sm:gap-2 flex-shrink-0 self-end sm:self-auto">
          ...
          <button
            onClick={() => onDelete(item.work_item_id)}
            disabled={isDeleting}
            className="p-2 hover:bg-slate-700 rounded-lg transition-colors text-slate-400 hover:text-red-400 disabled:opacity-50"
            title="Delete"
          >
            {isDeleting ? (
