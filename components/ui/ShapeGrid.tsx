...
const ShapeGrid: React.FC<ShapeGridProps> = ({
  ...
  const resolvedHoverFillColor = hoverFillColor ?? hoverColor ?? '#222';
  const resolvedFadeColor = fadeColor ?? '#120F17';

  useEffect(() => {
    const canvas = canvasRef.current;
    if (!canvas) return;
    const ctx = canvas.getContext('2d');
    const squareSize = resolvedSquareSize;
    const fillColor = resolvedHoverFillColor;
    const vignetteColor = resolvedFadeColor;
  ...
  return <canvas ref={canvasRef} className={`block h-full w-full border-none ${className ?? ''}`}></canvas>;
};
