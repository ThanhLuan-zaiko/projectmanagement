...
export default function ProjectsPageHeader({
  ...
  return (
    <section className="theme-hero-panel projects-bento-panel relative overflow-hidden rounded-[32px] p-6 backdrop-blur-xl sm:p-8">
            ...
            {highlights.length > 0 && (
              <div className="mt-5 grid gap-3 sm:grid-cols-3">
                {highlights.map((highlight) => (
                  <div
                    key={highlight.label}
                    className="projects-bento-subpanel rounded-2xl px-4 py-3 backdrop-blur"
                  >
                    <p className="projects-bento-kicker text-[11px] uppercase tracking-[0.2em]">{highlight.label}</p>
                    <p className="projects-bento-value mt-2 text-sm font-medium sm:text-base">{highlight.value}</p>
                  </div>
                ))}
              </div>
            )}
